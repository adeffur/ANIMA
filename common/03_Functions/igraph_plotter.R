...
igraph_plotter<-function(query.base,nodelist,edgetrips,rimpar,plot=TRUE,csv=FALSE,prefix=cytodir,filename="",lay_out=layout_with_dh(ig,maxiter=200,cool.fact=0.95,weight.node.dist=.65),optlabel="",optvalue="",optchar="",make.vertex.label=TRUE,return_graph=FALSE,plot_bipartite=FALSE){
  ...
  for (edgetrip in edgetrips){
    query.edges<-paste("RETURN DISTINCT ID(",edgetrip[1],") AS source, ID(",edgetrip[3],") AS target, ",edgetrip[2],".qvalue AS qvalue, ","TYPE(",edgetrip[2],") AS reltype, ",edgetrip[2],".Rsq AS RSQ, ",edgetrip[2],".weight AS PearsonR",sep="")
  ...
  ty<-c("baylor","cellEx","cellprop","ImmunePW","PalWangPW","pheno","PROBE","PROBETYPE","reactomePW","SYMBOL","wgcna","CELL","flowcellprop","cellpropFprop")
  tycol<-c("darkgoldenrod1","chartreuse1","chartreuse3","cadetblue1","cadetblue2","plum1","azure","gray88","cadetblue2","yellow","cornsilk2","seagreen2","springgreen2","springgreen3")
  ...
  plot(ig,vertex.shape="fcircle",vertex.label=vertexlabel,vertex.color=as.character(kindcolvector),layout=ig.lay,vertex.label.cex=.55,vertex.label.color="dimgray",vertex.frame.color=rimcol,vertex.frame.width=framewidth,edge.label=sprintf("%.5s",E(ig)$PearsonR),edge.label.cex=.7,xlim = c(-1.0, 1.0),ylim = c(-1.7, 1.0),margin=c(0,0,0,0),edge.arrow.mode=0)
  legend("bottomleft",levels(factor(V(ig)$kind)),fill=legstart,cex=.7,title="Vertex Type")
  legend("bottomright",sprintf("%.3f",seq(-extreme,extreme,length.out=11)),fill=as.character(numbers2colors(seq(-extreme,extreme,length.out=11),signed=TRUE,lim=c(-extreme,extreme),centered=T)),cex=.7,title=rimpar,ncol=2)
  ...
  if(plot_bipartite==TRUE){
    ...
    kindcolvector<-start
    plot(b1,vertex.label=V(b1)$nodename,vertex.color=as.character(kindcolvector),layout=b1.lay,vertex.label.cex=.7)
    ...
    kindcolvector<-start
    plot(b2,vertex.label=V(b2)$nodename,vertex.color=as.character(kindcolvector),layout=b2.lay,vertex.label.cex=.7)
  }
