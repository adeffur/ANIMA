superHeatmap2_rsc<-function(x,y=selected.probes,phenomatrix=phenomatrix,scale=scale,addTit=NULL,low=low,high=high,labRow=""){
...
heatmap_ad(as.matrix(exprs(x)[y,]),
           ...
           margins=c(9,5),
           RowSideColors=rsc,
           noan=2,labRow=labRow,verbose=TRUE   
)
}