...
probe_boxplot6<-function(squareC,generegex,probetype=FALSE,miny=6,maxy=14,orderP=c(1,2,3,4),fillvar){
  ...
  for (row in 1:nrow(boxplotdatalist[[1]])){
    ...
    print("final debug")
    colnames(allgd)<-c("probe","symbol","df","y","Categories")
    print(head(allgd))
    print("str(allgd)")
    print("class(allgd$Categories)")
    print(str(allgd))
    print(class(allgd$Categories))
    levelvec<-c(g1dname,g2dname,g3dname,g4dname)
    levelvec<-levelvec[orderP]
    allgd$Categories<-factor(allgd$Categories,levels=levelvec)
    #levels(allgd$Categories)<-levelvec
    print("levelvec")
    print(levelvec)
    
    # #boxplot comes here
    # nplot<-ggplot(allgd, aes(Categories,y)) +
    ...
    complevs<-eval(parse(text=paste("levels(as.factor(allgd$","Categories","))",sep="")))
    ...
    plotfig<-ggplot(allgd, aes(x=eval(parse(text=fillvar)), y=numeric, fill=eval(parse(text=fillvar)))) + 
    ...
    plotfig<-plotfig + facet_wrap(vars(probe),scales="free",ncol=3)
