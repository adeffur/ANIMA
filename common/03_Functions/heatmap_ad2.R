...
heatmap_ad2<-function (x, Rowv = NULL, Colv = if (symm) "Rowv" else NULL, 
    distfun = dist, hclustfun = hclust, reorderfun = function(d, 
        w) reorder(d, w), add.expr, symm = FALSE, revC = identical(Colv, 
        "Rowv"), scale = c("row", "column", "none"), na.rm = TRUE, 
    margins = c(5, 5), ColSideColors, RowSideColors, cexRow = 0.2 + 
        1/log10(nr), cexCol = 0.2 + 1/log10(nc), labRow = NULL, 
    labCol = NULL, main = NULL, xlab = NULL, ylab = NULL, keep.dendro = FALSE, noan=NULL,rowlab=TRUE, 
    verbose = getOption("verbose"), ...) 
{
    ...
    #lhei <- c((if (doCdend) 1 else 0.05) + if (!is.null(main)) 0.8 else 0,4)
    if (!missing(ColSideColors)) {
        ...
        lmat <- rbind(lmat[1, ] + 1, c(NA, 1), lmat[2, ] + 1)
         #lmat <- rbind(lmat[1, ] + 1, c(NA, 1), lmat[3, ] + 1)
    ...
    #5 draw col dendro 
    par(mar = c(0, 0, if (!is.null(main)) 2 else 0, margins[2]))#was1
    if (doCdend) 
    ...
    #6 title
    if (!is.null(main)) 
        title(main, cex.main = 1.3 * op[["cex.main"]])
