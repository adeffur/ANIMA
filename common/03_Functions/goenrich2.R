goenrich2<-function (labels, entrezCodes, yeastORFs = NULL, organism = "human", 
          ontologies = c("BP", "CC", "MF"), evidence = "all", includeOffspring = TRUE, 
          backgroundType = "givenInGO", removeDuplicates = TRUE, leaveOutLabel = NULL, 
          nBestP = 10, pCut = NULL, nBiggest = 0, getTermDetails = TRUE, 
          verbose = 2, indent = 0) 
{
  print("one")
  ...
  organisms = c("human", "mouse", "rat", "malaria", "yeast", 
                "fly", "bovine", "worm", "canine", "zebrafish", "chicken")
  allEvidence = c("EXP", "IDA", "IPI", "IMP", "IGI", "IEP", 
                  "ISS", "ISO", "ISA", "ISM", "IGC", "IBA", "IBD", "IKR", 
                  "IRD", "RCA", "TAS", "NAS", "IC", "ND", "IEA", "NR")
  ...
  print("three")
  orgCodes = c("Hs", "Mm", "Rn", "Pf", "Sc", "Dm", "Bt", "Ce", 
               "Cf", "Dr", "Gg")
  orgExtensions = c(rep(".eg", 4), ".sgd", rep(".eg", 6))
  reverseMap = c(rep(".egGO2EG", 4), ".sgdGO2ORF", rep(".egGO2EG", 
                                                       6))
  missingPacks = NULL
  packageName = paste("org.", orgCodes[orgInd], orgExtensions[orgInd], 
                      ".db", sep = "")
  ...
  if (orgInd == 5) {
    if (!is.null(yeastORFs)) {
      ...
      newCodes = paste("InvalidCode", c(1:length(entrezCodes)), 
                       sep = ".")
  ...
  if (orgInd == 5) {
    ...
    mapped_genes = as.numeric(do.call(match.fun("mappedkeys"), 
                                           list(egGO)))
  ...
  Go2eg = eval(parse(text = paste("AnnotationDbi::as.list(", 
                                  packageName, ":::org.", orgCodes[orgInd], reverseMap[orgInd], 
                                  ")", sep = "")))
  nTerms = length(Go2eg)
  print("eight")
  ...
  if (backT == 1) {
    ...
    else nBackgroundGenes = nMappedGenes
  }
  ...
  for (c in 1:nTerms) if (!is.na(Go2eg[[c]][[1]])) {
    ...
    if (includeOffspring) {
      ...
      if (length(termOffspring) > 0) {
        ...
        if (length(o2go) > 0) 
          for (o in 1:length(o2go)) if (!is.na(Go2eg[[o2go[o]]][[1]])) {
            newc = Go2eg[[o2go[o]]]
            newe = names(newc)
            newl = length(newe)
            if ((len + newl) > maxLen) {
    ...
    if (orgInd == 5) {
      ...
      else termCodes[[c]] = as.character(intersect(tc[use], 
                                                   mapECodes))
    }
    else {
      if (backT == 2) {
      ...
      else termCodes[[c]] = as.numeric(as.character(intersect(tc[use], 
                                                              mapECodes)))
    }
    nAllInTerm[c] = length(termCodes[[c]])
    if ((c%%50 == 0) & (verbose > 0)) 
      pind = updateProgInd(c/nTerms, pind)
  }
  if (verbose > 0) {
  ...
  for (set in 1:nSets) {
    if (verbose > 0) 
      printFlush(paste(spaces, " ..working on label set", 
                       set, ".."))
    ...
    if (backT == 1) {
      for (ll in 1:nLabelLevels) {
        modCodes[[ll]] = entrezCodes[labels[, set] == 
                                       labelLevels[ll]]
      ...
      for (ll in 1:nLabelLevels) {
        modCodes[[ll]] = mapECodes[mapLabels[, set] == 
                                     labelLevels[ll]]
        nModCodes[ll] = length(modCodes[[ll]])
      }
    }
    ...
    for (ll in 1:nLabelLevels) countsInTerm[ll, ] = sapply(lapply(termCodes, 
                                                                  intersect, modCodes[[ll]]), length)
    nAllInTermMat = matrix(nAllInTerm, nLabelLevels, nTerms, 
                           byrow = TRUE)
    nModCodesMat = matrix(nModCodes, nLabelLevels, nTerms)
    tabArr = array(c(countsInTerm, nAllInTermMat - countsInTerm, 
                     nModCodesMat - countsInTerm, nBackgroundGenes - nModCodesMat - 
                       nAllInTermMat + countsInTerm), dim = c(nLabelLevels * 
                                                                nTerms, 2, 2))
    if (verbose > 0) 
      printFlush(paste(spaces, "   ..calculating enrichments (this may also take a while).."))
    calculate = c(countsInTerm) > 0
    enrichment[calculate] = apply(tabArr[calculate, , ], 
                                  1, ftp, alternative = "g")
    dimnames(enrichment) = list(labelLevels, names(Go2eg))
    dimnames(countsInTerm) = list(labelLevels, names(Go2eg))
    bestPTerms = list()
    modSizes = table(labels[!(labels[, set] %in% leaveOutLabel), 
                            set])
    if (!is.null(pCut) || nBestP > 0) {
      ...
      for (ont in 1:nConsideredOntologies) {
        if (ont == nConsideredOntologies) {
          ontTerms = is.finite(match(termOntologies, 
                                     ontologies))
          bestPTerms[[ont]] = list(ontology = ontologies)
          names(bestPTerms)[ont] = paste(ontologies, 
                                         collapse = ", ")
        }
        else {
        ...
        for (ll in 1:nLabelLevels) {
          if (!is.null(pCut)) {
            reportTerms = c(1:nTerms)[ontTerms][ontEnr[ll, 
                                                       ] < pCut]
            reportTerms = reportTerms[order(ontEnr[ll, 
                                                   ][reportTerms])]
          }
          else reportTerms = c(1:nTerms)[ontTerms][order[1:nBestP, 
                                                         ll]]
          nRepTerms = length(reportTerms)
          enrTab = data.frame(module = rep(labelLevels[ll], 
                                           nRepTerms), modSize = rep(modSizes[ll], nRepTerms), 
                              bkgrModSize = rep(nModCodes[ll], nRepTerms), 
                              rank = c(1:nRepTerms), enrichmentP = enrichment[ll, 
                                                                              reportTerms], BonferoniP = pmin(rep(1, 
                                                                                                                  nRepTerms), enrichment[ll, reportTerms] * 
                                                                                                                nOntTerms), nModGenesInTerm = countsInTerm[ll, 
                                                                                                                                                           reportTerms], fracOfBkgrModSize = countsInTerm[ll, 
                                                                                                                                                                                                          reportTerms]/nModCodes[ll], fracOfBkgrTermSize = countsInTerm[ll, 
                                                                                                                                                                                                                                                                        reportTerms]/nAllInTerm[reportTerms], bkgrTermSize = nAllInTerm[reportTerms], 
                              termID = names(Go2eg)[reportTerms], termOntology = rep("NA", 
                                                                                     nRepTerms), termName = rep("NA", nRepTerms), 
          ...
          for (rci in 1:length(reportTerms)) {
            ...
            if (is.finite(dbind)) {
              ...
              enrTab$termDefinition[rci] = eval(parse(text = "AnnotationDbi:::Definition(goInfo[[dbind]])"))
            ...
            if (getTermDetails) {
              ...
              bestPTerms[[ont]]$forModule[[ll]][[rci]] = list(termID = termID, 
                                                              termName = enrTab$termName[rci], enrichmentP = enrTab$enrichmentP[rci], 
                                                              termDefinition = enrTab$termDefinition[rci], 
                                                              termOntology = enrTab$termOntology[rci], 
                                                              geneCodes = geneCodes, genePositions = keepEC[match(geneCodes, 
                                                                                                                  entrezCodes)])
            }
          }
          if (ll == 1) {
          ...
          else bestPTerms[[ont]]$enrichment = rbind(bestPTerms[[ont]]$enrichment, 
                                                    enrTab)
        }
      }
    }
    ...
    if (nBiggest > 0) {
      ...
      for (ont in 1:nConsideredOntologies) {
        if (ont == nConsideredOntologies) {
          ...
          names(biggestTerms)[ont] = paste(ontologies, 
                                           collapse = ", ")
        }
        else {
          ontTerms = termOntologies == ontologies[ont]
        ...
        for (ll in 1:nLabelLevels) {
          reportTerms = c(1:nTerms)[ontTerms][order[1:nBiggest, 
                                                    ll]]
          nRepTerms = length(reportTerms)
          enrTab = data.frame(module = rep(labelLevels[ll], 
                                           nRepTerms), modSize = rep(modSizes[ll], nRepTerms), 
                              ...
                              rank = c(1:nRepTerms), enrichmentP = enrichment[ll, 
                                                                              reportTerms], BonferoniP = pmin(rep(1, 
                                                                                                                  nRepTerms), enrichment[ll, reportTerms] * 
                                                                                                                nOntTerms), nModGenesInTerm = countsInTerm[ll, 
                                                                                                                                                           reportTerms], fracOfModSize = countsInTerm[ll, 
                                                                                                                                                                                                      reportTerms]/nModCodes[ll], fracOfBkgrTermSize = countsInTerm[ll, 
                                                                                                                                                                                                                                                                    reportTerms]/nAllInTerm[reportTerms], bkgrTermSize = nAllInTerm[reportTerms], 
                              termID = names(Go2eg)[reportTerms], termOntology = rep("NA", 
                                                                                     nRepTerms), termName = rep("NA", nRepTerms), 
          ...
          else biggestTerms[[ont]]$enrichment = rbind(biggestTerms[[ont]]$enrichment, 
                                                      enrTab)
        }
      }
    }
    setResults[[set]] = list(countsInTerm = countsInTerm, 
                             enrichmentP = enrichment, bestPTerms = bestPTerms, 
  ...
  kept[keepEC] = TRUE
  if (nSets == 1) {
    list(keptForAnalysis = kept, inGO = inGO, countsInTerms = setResults[[1]]$countsInTerm, 
         enrichmentP = setResults[[1]]$enrichmentP, bestPTerms = setResults[[1]]$bestPTerms, 
         biggestTerms = setResults[[1]]$biggestTerms)
  }
  else {
    list(keptForAnalysis = kept, inGO = inGO, setResults = setResults)
  }
}